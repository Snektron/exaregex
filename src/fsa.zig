...
pub const FiniteStateAutomaton = struct {
    const Self = FiniteStateAutomaton;
    ...
    pub const State = struct {
        ...
        /// forms an index into `FiniteStateAutomaton.transitions`.
    ...
    pub const Builder = struct {
        ...
        /// Finalize this state machine and turn it into a proper `FiniteStateAutomaton`.
        /// After this operation, the Builder is in a valid, empty state, and can be recycled
        /// or `deinit`ialized.
        pub fn build(self: *Builder, a: Allocator) !FiniteStateAutomaton {
            ...
            const states = try a.alloc(FiniteStateAutomaton.State, self.states.items.len);
            errdefer a.free(states);
            const transitions = try a.alloc(FiniteStateAutomaton.Transition, self.transitions.items.len);
...
test "FiniteStateAutomaton.Builder - empty" {
    var builder = FiniteStateAutomaton.Builder.init(testing.allocator);
    defer builder.deinit();

    var fsa = try builder.build(testing.allocator);
    defer fsa.deinit(testing.allocator);

    try testing.expectEqualSlices(FiniteStateAutomaton.State, &.{}, fsa.states);

    try testing.expectEqualSlices(FiniteStateAutomaton.Transition, &.{}, fsa.transitions);
}

test "FiniteStateAutomaton.Builder - simple" {
    var builder = FiniteStateAutomaton.Builder.init(testing.allocator);
    defer builder.deinit();
    ...
    try testing.expectEqualSlices(FiniteStateAutomaton.State, &.{
